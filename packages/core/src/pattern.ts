...
type ExtendCallback<Target, Props> = <Component>(string: TemplateStringsArray, ...properties: Properties<Inter<Component, Props>>[]) => Pattern<Target, Inter<Component, Props>>
...
const construct = (tag: string) => {
  return <Props>(stringOrPattern: Pattern<Target, Props> | TemplateStringsArray, ...properties: Properties<Props>[]): Pattern<Target, Props> | ExtendCallback<Target, Props> => {
    if ('__teiler__' in stringOrPattern) {
      return <Component>(strings: TemplateStringsArray, ...properties: Properties<Inter<Component, Props>>[]) => {
